...
def ocr_from_image(image_path, lang='chi_sim+eng', provider='tesseract', preprocess=True, tesseract_cmd=None, api_params=None):
    ...
    if provider == 'tesseract':
        ...
        # 检查 Tesseract 是否可用，提供友好报错信息
        try:
            # 如果 tesseract 未正确安装或路径错误，pytesseract 会抛出异常
            pytesseract.get_tesseract_version()
        except Exception as e:
            raise RuntimeError(
                "本地 Tesseract 未找到或不可用：请安装 Tesseract 并确保其在 PATH 中，或设置环境变量 TESSERACT_CMD 指向可执行文件路径。错误详情: " + str(e)
            )
