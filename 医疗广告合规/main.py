...
def main():
    ...
    parser.add_argument('--input', '-i', required=True, help='输入文本或图片路径')
    parser.add_argument('--type', '-t', choices=['text', 'image'], default='text', help='输入类型')
    parser.add_argument('--report', '-r', default=None, help='报告输出路径（默认 reports/report_<ts>.json）')
    parser.add_argument('--ocr-provider', choices=['tesseract', 'baidu', 'aliyun'], default=None, help='OCR 提供者，默认使用配置中的 OCR_PROVIDER_DEFAULT')
    parser.add_argument('--tesseract-cmd', default=None, help='如果 tesseract 不在 PATH，可通过此参数传入 tesseract.exe 的完整路径')
    ...
    if args.type == 'image':
        # OCR
        provider = args.ocr_provider or config.OCR_PROVIDER_DEFAULT
        ocr_text = ocr_from_image(input_path, tesseract_cmd=args.tesseract_cmd, provider=provider)
        input_text = ocr_text
    else:
        # 直接读取文本或把参数当作文本内容
